...
double countLocal(unordered_map<int, int> sidx2ip, vector<int> curres,vector<int>& itm_idx, ECDAG* ecdag){
    int count = 0;
    int total = 0;
    ...
    for(auto idx : itm_idx)
    {
        ...
        int nodeColor = coloring_res[node->getNodeId()];
        if(nodeColor == -1)
            continue;
        // cout << "node" << node->getNodeId() << " color = " << nodeColor << endl;
        vector<int> childColors;
        total ++;
        for (auto child : node->getChildNodes()) {
            int childcolor = coloring_res[child->getNodeId()];
            if(childcolor != -1){
                if(find(childColors.begin(), childColors.end(), childcolor) == childColors.end())
                    childColors.push_back(childcolor);
            }
        }
        if(find(childColors.begin(), childColors.end(), nodeColor) != childColors.end()){
            count++;
        }
    }
    // cout << count << "/" << total << "=" << count*1.0/total << endl;
...
void expand(Solution* current, int v, int m, unordered_map<string, bool>& visited,
    ...
    for (int i=0; i<v; i++) {
        ...
        for (int j=0; j<m; j++) {
            ...
            solution[i] = j;
            Solution* neighbor = new Solution(id++, v, m, solution);
                ...
                if (neighbor) {
                    if(!updateTradeoffCurve(tradeoff_curve_head, tradeoff_curve_tail, neighbor, load2bdwt, bdwt2load)) {
                        ...
                        neighbor = NULL;
                            ...
                            cout << neighbor->getId()  << " " << neighbor->getBdwt() << " "<< neighbor->getLoad() << " " << countLocal(sidx2ip, neighbor->getSolution(), itm_idx, ecdag) << endl;
...
Solution* genSol(vector<int> itm_idx, vector<int> candidates,
    ...
    while (true) {
        ...
        if (sol == NULL) {
            ...
            cout << sol->getId() << " "<< sol->getLoad() << " " << sol->getBdwt() << " " << countLocal(sidx2ip, sol->getSolution(), itm_idx, ecdag) << endl;
        }
...
int main(int argc, char** argv) {
  ...
  Solution* mlp = genSol(itm_idx, candidates, sidx2ip, ecdag, 0, w, k*w, realLeaves);
