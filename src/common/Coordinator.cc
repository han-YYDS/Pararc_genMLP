...
void Coordinator::degradeRead(CoorCommand* coorCmd) {
    ...
    if (method == "dist") {
        if (code == "Clay")
            repairBlockDist1(blockName, clientIp, true, false); // here
        else
...
void Coordinator::repairBlockDist1(string blockName, unsigned int clientip, bool enforceip, bool wait) {
    ...
    vector<string> blocklist = stripemeta->getBlockList();
    vector<unsigned int> loclist = stripemeta->getLocList(); // blocks location 
    ...
    if (enforceip) { // 指定了IP
        repairLoc = clientip;
        loclist[repairBlockIdx] = clientip; // new node
    } else { 
        repairLoc = loclist[repairBlockIdx]; // situ reconstruction 
        //repairLoc = selectRepairIp(loclist);
        //loclist[repairBlockIdx] = repairLoc;
    }
    ...
    // for leave vertices, if it is stored in block [i], then we initialize the color to i.
    // for header, we use the repairBlockIdx for its color
    // 对于所有叶子节点, 其block_index即为其color, 对于header,将修复块的idx作为其color
    unordered_map<int, int> sidx2bidx; // leaves and headers slices to block
    int shortnum = 0; // shorten
    for (auto sidx: ecLeaves) { 
        int bidx = sidx / ecw;
        if (bidx < ecn )
            sidx2bidx.insert(make_pair(sidx, bidx));
            ...
            shortnum++;
            sidx2bidx.insert(make_pair(sidx, -1));  // virtual blocks : -1
        }
    }
    for (auto sidx: ecHeaders) { // 疑惑: Concact之后,headers只有一个,那么这个还有意义吗?
        sidx2bidx.insert(make_pair(sidx, repairBlockIdx));
    }
...
//    cout << "num shortening: " << shortnum << endl;
    ...
    // 9. for intermediate vertices
    // note that virtual vertices for shortening are not counted as intermediate vertices
    vector<int> itm_idx; // intermediate indexs in Dag (not all in split)
    vector<int> candidates; // n nodes
    for (auto item: ecNodeMap) {
        int sidx = item.first;
        if (find(ecHeaders.begin(), ecHeaders.end(), sidx) != ecHeaders.end())
            continue;
    ...
    // 10. read the tradeoff point and calculate maxload and bdwt
    // 根据string对ecdag中的节点进行染色
    string tpentry = stripemeta->getCodeName() + "_" + to_string(ecn) + "_" + to_string(eck) + "_" + to_string(ecw);
    cout << "tpentry: " << tpentry << endl;
    TradeoffPoints* tp = _stripeStore->getTradeoffPoints(tpentry);
    vector<int> itm_coloring = tp->getColoringByIdx(repairBlockIdx); // MLP string
    ...
    int bdwt, maxload;
    // here
    stat(sidx2bidx, itm_coloring, itm_idx, ecdag, &bdwt, &maxload); // get bdwt and maxload generate by repair
    cout << "maxload: " << maxload << ", bdwt: " << bdwt << endl;

    // 11. generate coloring results
    unordered_map<int, unsigned int> coloring_res;
    for (auto item: sidx2bidx) { // headers and leaves
        int idx = item.first; // slice
        int color = item.second; // 非inm节点, 其blockIdx直接是其color
        unsigned int ip;
        if (color < 0)
            ip = 0;
        else
    ...
    for (int i=0; i<itm_idx.size(); i++) { // intermediate
        int idx = itm_idx[i];
        int color = itm_coloring[i]; // 染色结果
        unsigned int ip = loclist[color];
        coloring_res.insert(make_pair(idx, ip)); // 为intermediate分配物理node
    }
    ...
    cout << "blkbytes: " << blkbytes << ", pktbytes: " << pktbytes << endl;
    vector<ECTask*> tasklist;
    // 1.leaves-read  2.intermediate-compute  3.header-concact
    ecdag->genECTasksByECClusters(tasklist, ecn, eck, ecw, blkbytes, pktbytes, stripename, blocklist, coloring_res);
    cout << "tasklist: " << endl;
    for (int i=0; i<tasklist.size(); i++) {
       cout << "  " << tasklist[i]->dumpStr() << endl;
    ...
    for (auto agcmd: cmdlist) {
      ...
      char* cmdstr = agcmd->getCmd();
      int cmLen = agcmd->getCmdLen();
      char* todist = (char*)calloc(cmLen + 4, sizeof(char)); // ip+cmd
      memcpy(todist, (char*)&ip, 4);
      memcpy(todist+4, cmdstr, cmLen); 
      todelete.push_back(todist);
      redisAppendCommand(distCtx, "RPUSH dist_request %b", todist, cmLen+4);
    ...
    redisGetReply(distCtx, (void **)&distReply);
    freeReplyObject(distReply);
    for (auto item: todelete) { // 这里的todelete只是对其数量做了要求吗? 
      redisGetReply(distCtx, (void **)&distReply);
      freeReplyObject(distReply);
    }
    redisGetReply(distCtx, (void **)&distReply); // 怎么还有个? EXEC的吗?
    freeReplyObject(distReply);
    redisFree(distCtx);
...
void Coordinator::repairBlockListDist1Standby(vector<string> blocklist, unsigned int clientIp) {
    ...
    for (int i=0; i<blocklist.size(); i++) {
        repairBlockDist1(blocklist[i], clientIp, true, true); // 单块修复
    }
}
...
void Coordinator::repairNodeDist(unsigned int nodeip, string code, unordered_map<string, StripeMeta*> blk2meta) {
    ...
    for (auto item: blk2meta) {
        ...
        for (auto item: inmap) {
            if (item.second > load)
                load = item.second; // 最大load
        }

        if (load <= centnum) { // centnum为理论上所能接收的load上限
            // add to current batch
            curbatch.push_back(blk);
        } else { // 如果load超出界限,则将batch打包
