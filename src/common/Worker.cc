...
void Worker::readAndCache(AGCommand* agcmd) {
  ...
  unordered_map<int, int> cid2refs = agcmd->getCid2Refs(); // slice -> its reference
  ...
  vector<int> pattern; // 位图标记所读slice的offset
  vector<int> indices; // 所读slice的集合
  for (int i=0; i<ecw; i++)
    pattern.push_back(0);
  for (auto item: cid2refs) {
      int cid = item.first;
      indices.push_back(cid);
      int j = cid % ecw; // 这里的slice都是在一个块中吗?
      pattern[j] = 1;
  }
...
void Worker::fetchAndCompute(AGCommand* agcmd) {
  ...
  vector<int> cacheIndices; // 记录有ref的块
  for (auto item: cid2refs) {
      int cid = item.first;
  ...
  unordered_map<unsigned int, vector<int>> ip2cidlist; // 从指定ip所fetch的块的映射
  unordered_map<int, BlockingQueue<DataPacket*>*> fetchmap; // 为每个prev分配了一个queue
  for (int i=0; i<prevIndices.size(); i++) { // ip2cidlist
  ...
  // create fetchthreads, curfetchmap
  ...
  for (auto item: ip2cidlist) {
      ...
      for (auto cid: cidlist) { // 对于指定ip所需要fetch的
          curfetchmap.insert(make_pair(cid, fetchmap[cid])); // 建立映射
      }
...
void Worker::concatenate(AGCommand* agcmd) {
  ...
  for (int i=0; i<pktnum; i++) {
    for (int j=0; j<prevIndices.size(); j++) {
      DataPacket* curPkt = fetchQueue[j]->pop(); // 拿到数据
      int len = curPkt->getDatalen();
      if (len) {
        ofs.write(curPkt->getData(), len); // 拼接数据
      }
      else break;
      delete curPkt;
    }
...
void Worker::readWorker(BlockingQueue<DataPacket*>* readqueue, string blockname, int ecw, vector<int> pattern, int blkbytes, int pktbytes) {
  ...
  int subpktbytes = pktbytes / ecw; // slice size
  int pktnum = blkbytes / pktbytes;
  int readLen = 0, readl = 0;

  struct timeval time1, time2, time3;                                                                                                                                                                    
  gettimeofday(&time1, NULL); 
  for (int i=0; i<pktnum; i++) { // stripe
    for (int j=0; j<ecw; j++) { // slices
      if (pattern[j] == 0) // needed slices
        continue;
      // now we erad the j-th subpacket in packet i
      int start = i * pktbytes + j * subpktbytes; // offset in block 
      readLen = 0;
      DataPacket* curpkt = new DataPacket(subpktbytes); // 自定义传输包
...
void Worker::cacheWorker(BlockingQueue<DataPacket*>* cachequeue, vector<int> idxlist, int ecw, string keybase, int blkbytes, int pktbytes, unordered_map<int, int> cid2refs) {
  ...
  for (int i=0; i<pktnum; i++) {
    for (int j=0; j<idxlist.size(); j++) {
      ...
      string key = keybase+":"+to_string(idxlist[j]); // stripename + sliceIdx
...
void Worker::cacheWorker2(
  ...
  for (int i=0; i<pktnum; i++) {
      for (auto item: cacheMap) {
          int cid = item.first;
          BlockingQueue<DataPacket*>* queue = cacheMap[cid]; // 拿到计算结果
          int ref = cacheRefs[cid];

          string key = stripename + ":" + to_string(cid);

          DataPacket* curslice = queue->pop(); // current slice
          int len = curslice->getDatalen();
          char* raw = curslice->getRaw();
          int rawlen = len + 4; 
          for (int k=0; k<ref; k++) { // 使用到了几次就在redis中缓存几个dataPkt
              redisAppendCommand(writeCtx, "RPUSH %s %b", key.c_str(), raw, rawlen); count++;
          }
          delete curslice;
          if (i>1) {
...
void Worker::fetchWorker2(unordered_map<int, BlockingQueue<DataPacket*>*> fetchMap,
  ...
  if (loc == 0) {
    ...
    redisReply* rReply;
    redisContext* fetchCtx = RedisUtil::createContext(loc); // destIP 
    for (auto item: fetchMap) {
    ...
    for (int i=replyid; i<pktnum; i++) {
      //string key = keybase+":"+to_string(i);
        for (auto item: fetchMap) {
            ...
            char* content = rReply->element[1]->str;
            DataPacket* pkt = new DataPacket(content);
            item.second->push(pkt); // 在此填装fetchmap
            freeReplyObject(rReply);
        }
    }
...
void Worker::computeWorker2(unordered_map<int, BlockingQueue<DataPacket*>*> fetchMap,
  ...
  for (int i=0; i<pktnum; i++) { // 一次pkt粒度的compute

    // read from fetchmap
    for (auto item: fetchMap) { // ip:blk 是1对1的
        int curidx = item.first;
        cout << "get " << curidx << endl;
        DataPacket* curpkt = fetchMap[curidx]->pop(); // 弹出一个
        pktMap.insert(make_pair(curidx, curpkt)); 
        bufMap.insert(make_pair(curidx, curpkt->getData()));
    ...
    // prepare data pkt for writemap 准备,用作后续来cache
    // 这里还没有装载到cacheMap中去,只是调用其first(idx)
    for (auto item: cacheMap) {
        int curidx = item.first;
        DataPacket* curpkt = new DataPacket(subpktbytes); // 空的
        pktMap.insert(make_pair(curidx, curpkt));
        bufMap.insert(make_pair(curidx, curpkt->getData()));
    ...
    // iterate through ctlist (compute task list)
    for (int ctidx=0; ctidx<ctlist.size(); ctidx++) {
      ComputeTask* curtask = ctlist[ctidx];
      vector<int> srclist = curtask->_srclist;
      vector<int> dstlist = curtask->_dstlist;
      ...
      for (auto dstidx: dstlist) {
        if (bufMap.find(dstidx) == bufMap.end()) {
          char* tmpbuf = (char*)calloc(subpktbytes, sizeof(char)); // 为计算结果请求缓存
          bufMap.insert(make_pair(dstidx, tmpbuf));
        }
      }
      ...
      // prepare data buf 
      for (int bufIdx = 0; bufIdx < srclist.size(); bufIdx++) {
        int child = srclist[bufIdx]; // child index
        // check whether there is buf in databuf
        assert (bufMap.find(child) != bufMap.end());
        data[bufIdx] = bufMap[child]; // data指向buf
      }
      ...
      for (int codeBufIdx = 0; codeBufIdx < dstlist.size(); codeBufIdx++) {
        ...
        char* codebuf;
        assert(bufMap.find(target) != bufMap.end());
        code[codeBufIdx] = bufMap[target]; // code指向buf
        vector<int> curcoef = coefs[codeBufIdx];
        for (int j=0; j<col; j++) {
          matrix[codeBufIdx * col + j] = curcoef[j];
        }
    ...
    for (auto item: cacheMap) {
        int curidx = item.first;
        DataPacket* curpkt = pktMap[curidx];
        cacheMap[curidx]->push(curpkt); // 结果填装到cache
        pktMap.erase(curidx);
        bufMap.erase(curidx);
        if (i == 0)
            ignore.insert(make_pair(curidx, 1));
