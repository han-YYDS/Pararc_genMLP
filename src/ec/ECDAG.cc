...
void ECDAG::genECUnits()
{
    ...
    for (auto item : inref)
    {
        if (item.second == 0) // 没有入序的节点 - leaves
...
void ECDAG::genECCluster(unordered_map<int, int> coloring, int clustersize)
{
    ...
    // 0. generate inrefs and outrefs for each node 记录每个node的出入度
    unordered_map<int, int> inref;
    unordered_map<int, int> outref;
    for (auto item : _ecNodeMap)
    {
        ...
        for (auto cnode : childlist)
        {
            int cidx = cnode->getNodeId(); // child idx - child的出度
            if (outref.find(cidx) == outref.end())
                outref.insert(make_pair(cidx, 1));
            else
                outref[cidx]++;
        ...
        inref.insert(make_pair(nodeId, childlist.size())); // 入度
    }

    unordered_map<int, int> leaves;
    for (auto item : inref)
    {
        if (item.second == 0) // 入度为0即为leaves
            leaves.insert(make_pair(item.first, 1));
    }

    int starti = 0;
    while (leaves.size() > 0) // 层序遍历出口
    {
        if (leaves.size() == 1) // concatenate
        {
            bool root = false;
        ...
        // 这里的parent node 相当于所有的intermediate node
        ...
        for (int ui = 0; ui < curunitlist.size(); ui++)
        {
            ...
            if (inref[pnodeidx] == 0)
                leaves.insert(make_pair(pnodeidx, 1)); // 层序遍历
        }

        // cout << "current color2unitlist: " << endl;
        // 将一层中拥有相同color的unit聚合起来
        for (auto item : color2unitlist)
        {
            ...
            {
                string cstr = _ecUnitMap[tmpunit]->getChildStr(); // childs str
                if (cstr2unitlist.find(cstr) == cstr2unitlist.end()) // childs相同的unit,建立起映射
                {
            ...
            for (int cstridx = 0; cstridx < cstrlist.size();)
            {
                ...
                if (curclusterunits.size() + curunitlist.size() <= clustersize)
                {
                    ...
                    for (auto tmpu : curunitlist)
                        curclusterunits.push_back(tmpu); // 聚合
                    cstridx++;
                }
                else // 
                {
                                   ...
                                   unordered_map<int, unsigned int> coloring_res)
{
    ...
    unordered_map<int, vector<int>> bid2cids; // leaves所对应的块需要读入
    for (auto item : realleaves)
    {
        int cid = item.first; //
        int bid = cid / ecw;  // blockidx
        if (bid2cids.find(bid) == bid2cids.end())
        {
            vector<int> list;
            list.push_back(cid);
